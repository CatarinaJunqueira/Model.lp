...
def restricao_P9(model,omega,N,H,W,T):
    ...
    for o in range(len(N)): 
        for i in range(W[o]):
            for t in range(T[o]):
                for n in omega[o]: 
                    rest_names.append('restP9_'+str(i+1)+'_'+str(t+1)+'_'+str(o+1))
                    rhs.append(M)
                    cols = [] 
                    coefs = []
                ...
                for o in range(d):
                    ...
                    for u in range(d+1,P):
                        for a in range(d+1,u+1):
                            cols.append('w_'+str(o+1)+'_'+str(u+1)+'_'+str(a+1)+'_'+str(r+1)+'_'+str(c+1))
                            coefs.append(1.0)           
...
if __name__ == "__main__":
     ModeloIntegrado('InstanciaArtigo_II.mat')